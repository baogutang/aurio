...
describe('end to end through hourlyStationId', () => {
  const track = (id) => ({ source: 'netease', id: String(id), title: `Song ${id}`, artist: 'Artist' });

  it('airs the stitched clip on the upcoming free track', async () => {
    ...
    await vi.waitFor(() => {
      expect(db.getQueue()[1].segueTtsUrl).toMatch(/^\/imaging\/id-09-[0-9a-f]{12}\.mp3$/);
    });
  });
  ...
  it('falls back to the voice clip when ffmpeg is missing', async () => {
    ...
    await vi.waitFor(() => {
      expect(db.getQueue()[1].segueTtsUrl).toBe('/tts/hourly-voice.mp3');
    });
  });
});
