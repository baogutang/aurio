...
// chat-kind segment whose tracks joined the TAIL of the programme log instead
// of cutting the line. Server side this is op 'insert' + placement 'append'
// (or a plain 'append') with tracks actually added.
export function isHotlineAccepted(b: SegmentResult | null | undefined): boolean {
  if (!b || b.error) return false;
  if ((b.kind ?? 'chat') !== 'chat') return false;
  if (!b.queue || b.queue.length === 0) return false;
  return b.op === 'append' || (b.op === 'insert' && b.placement === 'append');
}
