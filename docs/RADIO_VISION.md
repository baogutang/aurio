...
| P5 | 血肉：从「一个台」到「一个人」（Claudio 差距） | 0.6.0 | 🟡 第一轮完成（2026-07-13）：故事引擎 / 台卡·歌卡·转写流 / 设置体验；剩今日节目单 + 声线切换 + Speaking 时刻 |
...
- [ ] 07:00 plan cron 升级为「为你定制今日电台」：读日历（会议 → 静默窗、运动 → 能量段、晚间 → 收束段）、天气、品味统计，产出结构化**今日计划**（静默窗列表 + 分段意图 + 开口时机），存 db
- [ ] **静默窗执行**：会议前 10 分钟至结束自动禁话（talk budget 之上的硬 override）；话密度、imaging liner 同受约束
- [ ] 《早安频率》开场在播出中「念」今日计划（「今天三个会，十一点前我少说话」）
- [ ] UI：今日节目单可视 —— 计划作为可见 artifact（播出钟弧段按分段着色 / 或节目单浮层）
- [ ] 分段语气参数进 shows / 计划（早晨更软、深夜更低），与 C 的按段换声线衔接
...
- [ ] 按 show / 计划分段的声线参数（voiceType / speed / emotion 已有配置钩子，接到 shows.json 与今日计划）
- [ ] liners / 整点报时用新音色重生成（既有阻塞项，凭证到位后一并做）
- [ ] 「Speaking…」时刻：她说话时的全宽波形可视化（voiceAnalyser 已有，把 sidechain 升级成可见事件）
