...
| P5 | 血肉：从「一个台」到「一个人」（Claudio 差距） | 0.6.0 | ✅ 开发完成（2026-07-13）：故事引擎 / 台卡·歌卡·转写流 / 设置体验 / 今日节目单 / 声线切换 / Speaking 时刻；待用户凭证：豆包真机验证 + liners 重生成 |
...
## 六 B、P5 · 血肉：从「一个台」到「一个人」（Claudio 差距，0.6.0）

> 来源：2026-07-13 对标抖音 @mmguo 的 Claudio（4·21，14 年 / 3247 首 / 3000+ 歌单蒸馏成私人 AI 电台；
> 架构与 Aurio 逐项相同——Node 本地服务器 + PWA、Claude 大脑、网易云、Fish TTS、飞书日程、OpenWeather、UPnP）。
> 判定：P0–P4 建成了电台的**骨骼**（时间线 / 磁带 / 判官——对方未见），差距在**血肉**：
> 一个真懂音乐的人、一场围绕你日程的节目、一把值得听的嗓子、一张脸。
> 用户拍板（2026-07-13）：四个方向全要；口播全面转向 Claudio 式长故事；掌故**只讲可验证的**；声音走豆包。

### A · 音乐故事引擎（可验证素材管线）——本阶段的心脏

- [x] **素材卡管线** `server/music/story.js`：从音源 API 拉取真实文本——网易云歌手简介 / 专辑文案 / 发行信息，QQ 音乐同类接口，Navidrome 标签——永久缓存（同 cue 的做法，按 artist / album 键）
- [x] **蒸馏**：素材首次拉取后一次 LLM 蒸馏成 5–8 条「可讲的事实」（每条附原文出处片段），缓存复用；蒸馏失败降级为原文摘录
- [x] **素材块升级**：prompt 的「歌曲素材」从歌词钩子扩展为完整故事卡（事实 + 歌词钩子 + 元数据 + 探测器事实），预算放宽到 ~25 行
- [x] **人设改写**：默认风格从「短句克制」转向「音乐故事主播」——2–4 句一个完整故事弧（掌故 → 感官承诺 → 听众的此刻）；编造「关于听众」的事实仍然绝禁；新增红线：**具体数字 / 年代 / 人名只能出自素材卡，卡里没有就不讲**
- [x] **判官配套**：say 上限 60→160 字（深夜档 120）；新增确定性类目 `fabricated_fact`——口播中出现的年份 / 专辑名必须在素材块文本中出现过，否则打回；LLM 裁判标准同步更新；voice bible 增补 3–4 条长故事示范（自创素材）
- [x] **话密度反转**：预算继续下调（说得更少），但每次开口都是正餐——大 break 锚定在歌与歌的边界（voice tracking 已支持预合成长口播）

### B · 今日节目单（日程化编排）

- [x] 07:00 plan cron 升级为「为你定制今日电台」：读日历（会议 → 静默窗、运动 → 能量段、晚间 → 收束段）、天气、品味统计，产出结构化**今日计划**（静默窗列表 + 分段意图 + 开口时机），存 db
- [x] **静默窗执行**：会议前 10 分钟至结束自动禁话（talk budget 之上的硬 override）；话密度、imaging liner 同受约束
- [x] 《早安频率》开场在播出中「念」今日计划（「今天三个会，十一点前我少说话」）
- [x] UI：今日节目单可视 —— 计划作为可见 artifact（播出钟弧段按分段着色 / 或节目单浮层）
- [x] 分段语气参数进 shows / 计划（早晨更软、深夜更低），与 C 的按段换声线衔接

### C · 声音（豆包默认）

- [ ] 豆包设为默认 voice provider 路径并真机验证（**等用户凭证**：`DOUBAO_TTS_APPID/TOKEN`）
  - [x] 设置面板已落地（2026-07-13）：豆包凭证/音色/语速/情感 + 四步申请指南 + 试听；日历向导式接入（本机/飞书/钉钉/企微/ICS 供应商卡 + 保存并测试 + 傻瓜式错误指引）
- [x] 按 show / 计划分段的声线参数（voiceType / speed / emotion 已有配置钩子，接到 shows.json 与今日计划）
- [ ] liners / 整点报时用新音色重生成（既有阻塞项，凭证到位后一并做）
- [x] 「Speaking…」时刻：她说话时的全宽波形可视化（voiceAnalyser 已有，把 sidechain 升级成可见事件）

### D · 人格与界面呈现

- [x] **台卡 / 档案卡**：头像（PixelPet）、台词式 bio、从品味画像自动生成的风格标签（taste-profile 已有数据）、ON AIR 24/7 · GENRES ∞ · LISTENER n
- [x] **对话歌卡**：点歌 / 推荐回复渲染成可点的歌曲卡片（点卡 = 热线急插通道）
- [x] 口播字幕升级为带时间戳的滚动转写流（0:05 / 0:11 式，素材已在 messages 里）
- [ ] 语音输入（mic）——候选，优先级最低，可后置

**验收：随机听三个 break，至少两个让你想截图发给朋友；开机那一分钟像 Claudio 视频里那样值得录屏。**
...
- **2026-07-13 · 口播哲学反转：从「短句克制」到「音乐故事主播」。** 推翻 P0 时代的 60 字上限（当时治的是「话密而空」；现在治「话短而瘪」）。新公式 = 更低的话密度 × 更长的正餐口播：说得更少，每次都值得。60→160 字，故事弧 2–4 句。
- **2026-07-13 · 掌故只讲可验证的。** Claudio 的「1971 尼龙弦吉他」是模型自由发挥，会张冠李戴。Aurio 的等价物必须出自素材卡（音源 API 的真实歌手简介 / 专辑文案，蒸馏后缓存）；判官新增确定性校验：口播中的年份 / 专辑名必须能在素材文本中找到。故事感与可验证性同时成立。
- **2026-07-13 · 声音默认豆包。** 克隆音色（Fish）留作后续特定节目声线的选项。
