...
| P2 | 节目语法（节目表 + 热线 + 栏目） | 一个月 | 🟡 节目表 / 话密度 / 周五回顾 / 探测器 / 情感 TTS 已落地；热线化与歌曲背景栏目待做 |
...
- [ ] 口播不像人话 —— 根因是大脑没有任何歌的「素材」：歌词只喂了 UI 从未进过 prompt。修法：正在播 / 刚播完 / 即将播的歌词钩子（开头句 + 副歌句）进 prompt + 人设与 voice bible 增加「引半句歌词接歌」示范
- [ ] 对话处理完后聊天框应自动关闭回到主界面（出错、用户正在输入、还有请求在途时不自动关）
