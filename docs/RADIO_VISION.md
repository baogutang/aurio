...
| P5 | 血肉：从「一个台」到「一个人」（Claudio 差距） | 0.6.0 | ☐ 方案已定（2026-07-13），待开工 |
...
- [ ] **素材卡管线** `server/music/story.js`：从音源 API 拉取真实文本——网易云歌手简介 / 专辑文案 / 发行信息，QQ 音乐同类接口，Navidrome 标签——永久缓存（同 cue 的做法，按 artist / album 键）
- [ ] **蒸馏**：素材首次拉取后一次 LLM 蒸馏成 5–8 条「可讲的事实」（每条附原文出处片段），缓存复用；蒸馏失败降级为原文摘录
- [ ] **素材块升级**：prompt 的「歌曲素材」从歌词钩子扩展为完整故事卡（事实 + 歌词钩子 + 元数据 + 探测器事实），预算放宽到 ~25 行
- [ ] **人设改写**：默认风格从「短句克制」转向「音乐故事主播」——2–4 句一个完整故事弧（掌故 → 感官承诺 → 听众的此刻）；编造「关于听众」的事实仍然绝禁；新增红线：**具体数字 / 年代 / 人名只能出自素材卡，卡里没有就不讲**
- [ ] **判官配套**：say 上限 60→160 字（深夜档 120）；新增确定性类目 `fabricated_fact`——口播中出现的年份 / 专辑名必须在素材块文本中出现过，否则打回；LLM 裁判标准同步更新；voice bible 增补 3–4 条长故事示范（自创素材）
- [ ] **话密度反转**：预算继续下调（说得更少），但每次开口都是正餐——大 break 锚定在歌与歌的边界（voice tracking 已支持预合成长口播）
...
- [ ] **台卡 / 档案卡**：头像（PixelPet）、台词式 bio、从品味画像自动生成的风格标签（taste-profile 已有数据）、ON AIR 24/7 · GENRES ∞ · LISTENER n
- [ ] **对话歌卡**：点歌 / 推荐回复渲染成可点的歌曲卡片（点卡 = 热线急插通道）
- [ ] 口播字幕升级为带时间戳的滚动转写流（0:05 / 0:11 式，素材已在 messages 里）
